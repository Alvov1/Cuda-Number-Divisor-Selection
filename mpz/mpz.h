...
__device__ __host__ inline char* mpz_get_str(const mpz_t *mpz, char *str, int bufsize);
...
__device__ __host__ inline int mpz_is_negative(const mpz_t *mpz) {
...
__device__ __host__ inline void mpz_set_str(mpz_t *mpz, const char *user_str) {
  ...
  for (i = 0; i < num_digits; i ++) {
    ...
    digit_t d = strtoul(start, NULL, 16);
...
__device__ __host__ inline char* mpz_get_str(const mpz_t *mpz, char *str, int bufsize) {
  ...
  static constexpr auto printHex = [] (unsigned number, char* toBuffer) {
      char buffer[9] = {'0', '0', '0', '0', '0', '0', '0', '0', '\0'};
      const char* hexDigits = "0123456789abcdef";
      uint8_t bufferPosition = 0;

      while(number > 0 && bufferPosition < 8) {
          buffer[bufferPosition++] = hexDigits[number % 16];
          number /= 16;
      }

      for(uint8_t i = 0; i < bufferPosition; ++i)
          toBuffer[i] = buffer[bufferPosition - i - 1];
      return bufferPosition;
  };
  static constexpr auto printHexPadded = [](unsigned number, char* toBuffer) {
      char buffer[9] {};
      const char* hexDigits = "0123456789abcdef";
      uint8_t bufferPosition = 0;
  ...
  for (i = mpz->capacity - 1; i >= 0; i--) {
    ...
    if (digit != 0 || print_zeroes) {
      ...
      if (!print_zeroes) {
        str_index += printHex(digit, str + str_index);
//        str_index += sprintf(str + str_index, "%x", digit);
      }
      else {
        str_index += printHexPadded(digit, str + str_index);
